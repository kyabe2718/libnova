...
struct when_all_awaitable : TaskContainer {
...
protected:
    ...
    auto result() const &noexcept {
        if constexpr (!std::is_void_v<R>) {
            ...
            std::transform(immediate_tasks.begin(), immediate_tasks.end(), std::back_inserter(res),
                           [](auto &t) -> result_element_type { return t.result(); });
            std::transform(defer_tasks.begin(), defer_tasks.end(), std::back_inserter(res),
                           [](auto &t) -> result_element_type { return t.result(); });
            return res;
        }
    }
    ...
    auto result() const &&noexcept {
        if constexpr (!std::is_void_v<R>) {
            std::vector<result_element_type> res;
            res.reserve(immediate_tasks.size() + defer_tasks.size());
            std::transform(std::make_move_iterator(immediate_tasks.begin()), std::make_move_iterator(immediate_tasks.end()), std::back_inserter(res),
                           [](auto &t) -> result_element_type { return t.result(); });
            std::transform(std::make_move_iterator(defer_tasks.begin()), std::make_move_iterator(defer_tasks.end()), std::back_inserter(res),
                           [](auto &t) -> result_element_type { return t.result(); });
            return res;
        }
    }
